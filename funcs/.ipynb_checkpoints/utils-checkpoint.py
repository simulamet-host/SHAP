...
def shap_ranking_table(X_train, shap_values_ori, shap_values_xm, shap_values_mi, shap_values_mice,
    ...
    combined_feature_importance = np.hstack((np.array(feature_importance_ori),np.array(feature_importance_xm),
                                             np.array(feature_importance_mi),
                                             np.array(feature_importance_mice),np.array(feature_importance_dimv),
                                            np.array(feature_importance_mf),np.array(feature_importance_soft),))
    ...
    combined_feature_importance = pd.concat([feature_importance_ori, feature_importance_xm, feature_importance_mi,
              ...
              feature_importance_mf, feature_importance_soft], axis = 1)
    print('combined_feature_importance')
    combined_feature_importance = combined_feature_importance.drop(combined_feature_importance.columns[[2,4,6,8]], axis = 1) 
    combined_feature_importance.columns = ['Original','Xgb on missing data','Mean Imputation', 'MICE','DIMV','missForest','SOFT-IMPUTE']
